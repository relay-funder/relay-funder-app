...
**Important Note:** The `BRIDGE_API_KEY` constant uses the `NEXT_PUBLIC_BRIDGE_CLIENT_ID`. The `BridgeService` uses this key directly in the `Authorization` header _without_ the `Bearer` prefix.
