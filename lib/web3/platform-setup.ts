...
export async function ensurePlatformSetup(): Promise<{
  ...
  try {
    ...
    const keepWhatsRaisedImpl =
      process.env.NEXT_PUBLIC_KEEP_WHATS_RAISED_IMPLEMENTATION;
    ...
    if (keepWhatsRaisedImpl) {
      ...
      const treasuryFactoryWithSigner = treasuryFactory.connect(
        platformAdminSigner,
      ) as typeof treasuryFactory;

      const registerTx =
        await treasuryFactoryWithSigner.registerTreasuryImplementation(
          platformHash,
      ...
      const approveTx =
        await treasuryFactoryWithProtocol.approveTreasuryImplementation(
          platformHash,
          0,
        );
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error.message : 'Unknown platform setup error',
    };
  }
}
...
export async function validatePlatformSetup(): Promise<{
  ...
  try {
    ...
    try {
      ...
      if (code === '0x') {
        issues.push(
          `GlobalParams contract not found at address ${globalParamsAddress}`,
        );
        return { isValid: false, issues };
      }
    } catch (error) {
      issues.push(
        `Failed to check GlobalParams contract: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
      return { isValid: false, issues };
    }
    ...
    try {
      ...
      if (
        ...
        issues.push(
          `Failed to check platform status: ${error instanceof Error ? error.message : 'Contract call failed'}`,
        );
        return { isValid: false, issues };
      }
    }
    ...
    if (!isListed) {
      if (platformCheckFailed) {
        issues.push(
          'Platform validation failed: Unable to verify enlistment status. Platform may need to be enlisted manually.',
        );
      } else {
