...
async function fetchCampaignPage({ pageParam = 1, status = 'active', pageSize = 10 }) {
...
export function useInfiniteCampaigns(status = 'active', pageSize = 10) {
    return useInfiniteQuery<PaginatedResponse, Error>({
        ...
        queryFn: ({ pageParam = 1 }) => fetchCampaignPage({ pageParam: pageParam as number, status, pageSize }),
        ...
        getPreviousPageParam: (firstPage: PaginatedResponse) => 
            firstPage.pagination.currentPage > 1 ? firstPage.pagination.currentPage - 1 : undefined,
        initialPageParam: 1,
    })
}
...
export async function prefetchCampaigns(queryClient: QueryClient, status?: string) {
