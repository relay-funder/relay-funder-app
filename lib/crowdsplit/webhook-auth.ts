...
export function validateCrowdSplitWebhookAuth(
  ...
  if (payloadSecret) {
    const isPayloadValid = validatePayloadSecret(payloadSecret, CROWDSPLIT_WEBHOOK_SECRET);
    if (isPayloadValid) {
  ...
  if (signatureHeader) {
    const isSignatureValid = validateHmacSignature(signatureHeader, body, CROWDSPLIT_WEBHOOK_SECRET);
    if (isSignatureValid) {
  ...
  if (stripeSignature) {
    const isStripeValid = validateStripeStyleSignature(stripeSignature, body, CROWDSPLIT_WEBHOOK_SECRET);
    if (isStripeValid) {
      debug && console.log('✅ Stripe-style signature authentication successful');
      return {
        isValid: true,
        method: 'stripe_style',
  ...
  if (!payloadSecret && !signatureHeader && !stripeSignature) {
    throw new ApiParameterError('Missing webhook authentication (no secret in payload or signature headers)');
  }
 ...
 */
function validateHmacSignature(signature: string, payload: string, secret: string): boolean {
  try {
 ...
 */
function validateStripeStyleSignature(signature: string, payload: string, secret: string): boolean {
  try {
