...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    request.headers.forEach((value, key) => {
      ...
      if (
        possibleSignatureHeaders.some(
          (h) =>
            keyLower.includes(h.toLowerCase()) ||
      ...
      if (sensitiveHeaders.some((sensitive) => keyLower.includes(sensitive))) {
        console.log(
          `  ${key}: ${value.substring(0, 20)}... (truncated for security)`,
        );
      } else {
    ...
    console.log(
      `  Has CROWDSPLIT_WEBHOOK_SECRET: ${!!CROWDSPLIT_WEBHOOK_SECRET}`,
    );
    console.log(
      `  CROWDSPLIT_WEBHOOK_SECRET value: ${CROWDSPLIT_WEBHOOK_SECRET ? CROWDSPLIT_WEBHOOK_SECRET.substring(0, 10) + '...' : 'Not set'}`,
    );

    // Check for secret in payload
    const payloadSecret = body?.secret;
    console.log(
      `  Secret in payload: ${payloadSecret ? payloadSecret : 'None'}`,
    );

    // Try all possible signature headers
    const allPossibleSignatures = possibleSignatureHeaders
      .map((headerName) => ({
        name: headerName,
    ...
    console.log(
      `  All signature-like headers found: ${allPossibleSignatures.length}`,
    );
    ...
    if (CROWDSPLIT_WEBHOOK_SECRET && payloadSecret) {
      if (CROWDSPLIT_WEBHOOK_SECRET === payloadSecret) {
        ...
        authenticationDetails =
          'Payload secret does not match environment variable';
    ...
    if (
      !isSignatureValid &&
      allPossibleSignatures.length > 0 &&
      ...
      for (const { name, value: signatureHeader } of allPossibleSignatures) {
        ...
        try {
          ...
          if (
            signatureHeader === expectedSignatureSha1 ||
            signatureHeader === `sha1=${expectedSignatureSha1}`
          ) {
            isSignatureValid = true;
        ...
        if (transactionId) {
          ...
          try {
            ...
            const existingPayment = await prisma.payment.findFirst({
              where: {
                OR: [{ externalId: transactionId }, { provider: 'CROWDSPLIT' }],
              },
            ...
            if (existingPayment) {
              console.log(
                `  ✅ Found payment: ID ${existingPayment.id}, Status: ${existingPayment.status}`,
              );
              console.log(
                `  📋 Payment details: External ID: ${existingPayment.externalId}, Amount: ${existingPayment.amount}`,
              );
              ...
              console.log(
                `  🔄 Would update status from '${existingPayment.status}' to '${newStatus}'`,
              );
              console.log(
                `  🧪 [DEBUG MODE] Payment update simulation completed`,
              );
            } else {
              console.log(
              ...
              console.log(
                `  📊 Recent CROWDSPLIT payments (${recentPayments.length}):`,
              );
              recentPayments.forEach((p) => {
                console.log(
                  `    - ID: ${p.id}, External: ${p.externalId}, Status: ${p.status}, Created: ${p.createdAt.toISOString()}`,
                );
              });
            }
          } catch (dbError) {
            console.log(
              `  ❌ Database lookup error: ${dbError instanceof Error ? dbError.message : 'Unknown error'}`,
            );
          }
        } else {
    ...
    return NextResponse.json(
      {
        ...
        debug: {
          ...
          receivedData: {
            ...
            eventType:
              body?.data?.type || body?.event || body?.type || 'unknown',
          },
          authentication: {
            ...
            payload_secret_matches: !!(
              CROWDSPLIT_WEBHOOK_SECRET &&
              payloadSecret &&
              CROWDSPLIT_WEBHOOK_SECRET === payloadSecret
            ),
          },
          simulation: {
            payment_processing: !!(
              body?.data?.type === 'transaction.updated' ||
