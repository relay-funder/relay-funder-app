...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (signatureHeader) {
      console.log(
        `  Signature source: ${['x-crowdsplit-signature', 'stripe-signature', 'x-signature'].find((h) => request.headers.get(h)) || 'query parameter'}`,
      );
    }
    ...
    if (CROWDSPLIT_WEBHOOK_SECRET && signatureHeader && bodyText) {
      try {
        ...
        if (timestampElement && signatureElement) {
          ...
          console.log(
            `  Signature validation: ${isSignatureValid ? '✅ VALID' : '❌ INVALID'}`,
          );
          ...
          console.log(
            `  Timestamp difference: ${timeDiff} seconds (${timeDiff > 300 ? 'STALE' : 'FRESH'})`,
          );
        } else {
          ...
          console.log(
            `  Simple HMAC validation: ${isSignatureValid ? '✅ VALID' : '❌ INVALID'}`,
          );
          console.log(`  ${signatureValidationDetails}`);
        }
        ...
        if (transactionId) {
          ...
          try {
            ...
            if (existingPayment) {
              ...
              const newStatus =
                status === 'completed' || type === 'payment_intent.succeeded'
                  ? 'confirmed'
                  : 'pending';
              console.log(
              ...
              recentPayments.forEach((p) => {
                console.log(
                  `    - ID: ${p.id}, External: ${p.externalId}, Status: ${p.status}`,
                );
              });
            }
          } catch (dbError) {
