...
export async function GET(
    ...
    try {
        ...
        const baselineCampaign = baselineDistribution.distribution.find(c => c.id === campaignId);
        const baselineMatch = baselineCampaign ? parseUnits(baselineCampaign.matchingAmount, state.tokenDecimals) : 0n;
        ...
        const currentContribution = campaign.aggregatedContributionsByUser[userId]
            ? parseUnits(campaign.aggregatedContributionsByUser[userId], state.tokenDecimals)
            : 0n;
        ...
        campaign.aggregatedContributionsByUser[userId] = formatUnits(newTotalContribution, state.tokenDecimals);
        ...
        const simulatedCampaign = simulatedDistribution.distribution.find(c => c.id === campaignId);
        const simulatedMatch = simulatedCampaign ? parseUnits(simulatedCampaign.matchingAmount, state.tokenDecimals) : 0n;
        const marginalMatch = simulatedMatch - baselineMatch;

        return response({
            estimatedMatch: formatUnits(simulatedMatch, state.tokenDecimals),
            marginalMatch: formatUnits(marginalMatch > 0n ? marginalMatch : 0n, state.tokenDecimals)
        });
