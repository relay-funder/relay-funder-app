...
const registerSchema = z.object({
    ...
    walletAddress: z.string().regex(/^0x[a-fA-F0-9]{40}$/, "Invalid Ethereum address"), // Submitter's wallet
    ...
    try {
        ...
        if (txHash && onchainRecipientId) {
            if (!existingRegistration) {
                ...
                console.error(`Attempted to update non-existent RoundCampaigns record for Round ${roundId}, Campaign ${campaignId}`)
                return NextResponse.json(
                    { success: false, error: "Pending registration record not found for update." },
                    { status: 404 }
                )
            }
            ...
            if (existingRegistration.status !== RecipientStatus.PENDING) {
                 return NextResponse.json(
                    { success: false, error: `Cannot update registration with status: ${existingRegistration.status}` },
            ...
            console.log(`Updated RoundCampaigns ${updatedRegistration.id} to APPROVED for Round ${roundId}, Campaign ${campaignId}`)
        ...
        // --- Handle Initial Registration Request (txHash and onchainRecipientId are NOT present) ---
            ...
            if (existingRegistration) {
                // Handle cases where a record already exists
                 ...
                 return NextResponse.json(
                    { success: false, message: message, status: existingRegistration.status },
                    { status: 409 } // Conflict
                )
            }
            ...
            console.log(`Created new PENDING RoundCampaigns for Round ${roundId}, Campaign ${campaignId}`)
            return NextResponse.json({ success: true, data: newRegistration }, { status: 201 }) // 201 Created
        }
        ...
        if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
             errorMessage = "A registration record for this campaign and round already exists."
